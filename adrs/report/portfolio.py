...
#             long_alphas=len(
#                 list(filter(lambda a: a.kind == AlphaKind.LONG, portfolio.alphas))
#             ),
#             short_alphas=len(
#                 list(filter(lambda a: a.kind == AlphaKind.SHORT, portfolio.alphas))
#             ),
#             both_alphas=len(
#                 list(filter(lambda a: a.kind == AlphaKind.BOTH, portfolio.alphas))
#             ),
...
class MultiAssetPortfolioReportV1Performance(BaseModel):
    performance: MultiAssetPortfolioPerformance
    performance_df: DataFrame[MultiAssetPortfolioPerformanceDF]


class MultiAssetPortfolioReportV1(BaseModel):
    portfolio_id: str
    # long_alphas: int
    # short_alphas: int
    # both_alphas: int
    back: MultiAssetPortfolioReportV1Performance
    forward: MultiAssetPortfolioReportV1Performance

    @staticmethod
    def version() -> str:
        return "multi-asset-portfolio-report-v1"
    ...
    def compute(
        ...
        return cls(
            ...
            back=MultiAssetPortfolioReportV1Performance(
                performance=back, performance_df=back_df
            ),
            forward=MultiAssetPortfolioReportV1Performance(
                performance=forward, performance_df=forward_df
            ),
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, MultiAssetPortfolioReportV1):
            return False
        return (
            self.portfolio_id == other.portfolio_id
            # and self.long_alphas == other.long_alphas
    ...
    def deserialize(cls, buf: bytes) -> Self:
        ...
        return cls(
            ...
            back=MultiAssetPortfolioReportV1Performance(
                **json.loads(df["back"][0]),
                performance_df=MultiAssetPortfolioPerformanceDF.validate(back_pdf),
            ),
            forward=MultiAssetPortfolioReportV1Performance(
                **json.loads(df["forward"][0]),
                performance_df=MultiAssetPortfolioPerformanceDF.validate(forward_pdf),
            ),
        )
